...
const validate = ()=>{
  ...
  &&arr!=''&&password!=''&&email!='') {
    ...
    .then((userCredential) => {
      ...
      const userId = email.replace('@','o').replace('.','o')
       ...
       push(ref(db, 'users/'), user).then(
      (e)=>{
        setStarted(false)
        set(ref(db, 'users/'+e.key+'/key') , e.key)
        
        localStorage.setItem('userKey', e.key)
      f7.views.main.router.navigate('/confirm/')
      localStorage.setItem('created',email)
  ...
  localStorage.setItem('user',JSON.stringify(user))
   ...
   lastUser.pic = localStorage.getItem('userProfile')
...
return(
     ...
     <ListInput
        ...
        <ListItem title="Votre sexe" smartSelect
                 smartSelectParams={{closeOnSelect: true, searchbar: false, 
        openIn: 'popup',
...
export default DemandeFormPage;
