...
if (localStorage.getItem('worker')!=null) {
  const worker = JSON.parse(localStorage.getItem('worker')).email
   workerId = worker.replace(/[^\w\s]/gi, "");
}
const ChatPresta= ({onCloseChat}) => {
  ...
  useEffect(() => {
    ...
    const messagesRef = ref(db, 'messages/'+workerId+'/list');
  ...
  const sendMessage = () => {
    ...
    if (text.length) {
      ...
      const sender = JSON.parse(localStorage.getItem('worker'))
      const workerId = sender.email.replace(/[^\w\s]/gi, "");
        const dataToSend= {
          sender:sender,
          IamWorker: true,
          type:'sent',
      ...
      set(ref(db, 'chat/'+workerId),dataToSend)
      push(ref(db, 'messages/'+workerId+'/list'),dataToSend).then(
     
      ).catch(
    ()=>{
      f7.dialog.alert('Envoi échoué')
    ...
    <Messagebar
      ...
      <Messages>
        ...
        {messagesData.map((message, index) =>
        {
          ...
          if (message.sender.email==worker) {
            type='sent'
          }else{
            type = 'received'
          }
       if (message.sender.email==worker||message.receiver.email==worker) {
        return(
